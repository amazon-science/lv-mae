...
def get_args_parser():
    parser = argparse.ArgumentParser('LV-MAE downstream task - linear probing', add_help=False)
    ...
    parser.add_argument('--lr', type=float, default=3e-4, help='learning rate for linear probing eval')
...
class LinearProbe(nn.Module):
    ...
    def forward(self, x, lengths, attn_mask=None):
        B, T, C = x.shape
        ...
        gap_features = []
        for i in range(B):
            valid_seq = x[i, 1:lengths[i].item()+1, :]  # Extract valid part, shape: [seq_len_i, D]
            gap_f = valid_seq.mean(dim=0)
            gap_features.append(gap_f)

        gap_features = torch.stack(gap_features)
        return self.linear(gap_features)


def train_one_epoch_lp(linear_probe, dataloader, optimizer, criterion, device):

    linear_probe.train()  # We only train the linear layer
    ...
    for batch in dataloader:
        ...
        logits = linear_probe(inputs, lengths, attn_mask)
...
def evaluate(linear_probe, dataloader, criterion, device):
...
def linear_probing(args, model, data_loader_train, data_loader_val, data_loader_test, num_classes, device):

    # Initialize the linear probing layer
    input_dim = args.input_size
    linear_probe = LinearProbe(model=model.forward_encoder, input_dim=input_dim, num_classes=num_classes).to(device)

    # Loss and optimizer
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(linear_probe.parameters(), lr=args.lr, amsgrad=True, weight_decay=1e-3)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)

    # Train and evaluate
    num_epochs = args.epochs  # Set your desired number of epochs
    accuracy = 0.
    test_accuracy = 0.
    test_avg_loss = -1.
    for epoch in tqdm(range(num_epochs)):
        print(f"Epoch {epoch + 1}/{num_epochs}")
        train_one_epoch_lp(linear_probe, data_loader_train, optimizer, criterion, device)
        print("Validation performance:")
        val_avg_loss_, val_accuracy_ = evaluate(linear_probe, data_loader_val, criterion, device)
        print(f"{Colors.OKGREEN}Our Test performance:")
        avg_loss_, accuracy_ = evaluate(linear_probe, data_loader_test, criterion, device)
    ...
    return test_avg_loss, test_accuracy, accuracy
...
avg_loss, lp_accuracy, lp_val_acc = linear_probing(args, model, data_loader_train, data_loader_val, data_loader_test, num_classes, args.device)
res_dict[args.task] = {'ft_accuracy': lp_accuracy}
